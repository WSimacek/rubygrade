...
# (*Optional*; edit the config/config.yml file to suit your needs)
# Open a shell and run @rake gems:install@ to install required gems, then @rake db:migrate@. (Optional; run @rake db:data:load@ to populate the database with sample data. 
...
To report a bug or make a suggestion head over to:
 ...
 "http://rubygrade.heroku.com/dashboard/comments":http://rubygrade.heroku.com/dashboard/comments
